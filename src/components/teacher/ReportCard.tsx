...
function ReportCard({ title, status, detail }: ReportCardProps) {
  const config = {
    good: { color: "#16a34a", bg: "#f0fdf4", icon: "OK", label: "Covered" },
    warning: { color: "#d97706", bg: "#fffbeb", icon: "!", label: "Partial" },
    missing: { color: "#dc2626", bg: "#fef2f2", icon: "X", label: "Missing" },
  }[status];
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: "14px", background: "white", border: "1px solid #e5e7eb", borderRadius: "12px", padding: "18px 20px" }}>
      <div style={{ width: "28px", height: "28px", borderRadius: "50%", background: config.bg, color: config.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "12px", fontWeight: 700, flexShrink: 0 }}>
        {config.icon}
      </div>
      <div>
